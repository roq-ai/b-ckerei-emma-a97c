...
import { getBakeryById, updateBakeryById } from 'apiSdk/bakeries';
...
function BakeryEditPage() {
  ...
  const handleSubmit = async (values: BakeryInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateBakeryById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<BakeryInterface>({
    initialValues: data,
    validationSchema: bakeryValidationSchema,
