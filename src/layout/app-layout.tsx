...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['business-owner', 'bakery-staff', 'store-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<BakeryInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/bakeries` : null),
    () => getBakeries({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Bakery',
      path: isTenantUser ? `/bakeries/view/${data?.[0]?.id}` : '/bakeries',
      entity: 'bakery',
      service: AccessServiceEnum.PROJECT,
      icon: FiCoffee,
    },
    { name: 'Product', path: '/products', entity: 'product', service: AccessServiceEnum.PROJECT, icon: FiShoppingBag },
